 ...
 * @author gregorro
 */
public class AccelerometerSensorDeviceManager extends AbstractWebService {
    ...
    public void processRequest(OutputStream o, HttpExchange t) throws IOException {
        ...
        try {
            AccelerometerSensorDevice device = (AccelerometerSensorDevice)r.getDevice(Integer.parseInt(requestParameters.get("deviceId")));
                ...
                if (user.getGroup().equals(Group.ADMIN_GROUP)) {
                    if (requestParameters.get("all").equals("true")) {
                        for (AbstractDevice dev : r.getDevices()) {
                            if (dev instanceof AccelerometerSensorDevice) {
                                for (int i=0; i<14;i++) {
                                    ((AccelerometerSensorDevice)dev).getAlarmScheduler().saveSchedule(requestParameters.get(Integer.toString(i)), Integer.toString(i));
                                }
                                if (((AccelerometerSensorDevice)dev).isEnabledAlarmScheduler()) ((AccelerometerSensorDevice)dev).setAlarmEnabled(((AccelerometerSensorDevice)dev).getAlarmScheduler().getCurrentSchedule(), true);
                                r.getPersistance().saveDevice(((AccelerometerSensorDevice)dev));                                                        
                            }
                        }
                        sendAjaxAnswer("OK");
                    } else {
                        ...
                        if (device.isEnabledAlarmScheduler()) device.setAlarmEnabled(device.getAlarmScheduler().getCurrentSchedule(), true);
                        r.getPersistance().saveDevice(device);
                        sendAjaxAnswer("OK");                        
                    }
                } else {
                ...
                if (user.getGroup().equals(Group.ADMIN_GROUP)) {
                    if (requestParameters.get("all").equals("true")) {
                        for (AbstractDevice dev : r.getDevices()) {
                            if (dev instanceof AccelerometerSensorDevice) {
                                for (int i=0; i<14;i++) {
                                    ((AccelerometerSensorDevice)dev).getSwitchScheduler().saveSchedule(requestParameters.get(Integer.toString(i)), Integer.toString(i));
                                }
                                if (((AccelerometerSensorDevice)dev).isEnabledSwitchScheduler()) ((AccelerometerSensorDevice)dev).setSwitchEnabled(((AccelerometerSensorDevice)dev).getSwitchScheduler().getCurrentSchedule(), true);
                                r.getPersistance().saveDevice(((AccelerometerSensorDevice)dev));                                                        
                            }
                        }
                        sendAjaxAnswer("OK");
                        ...
                        if (device.isEnabledSwitchScheduler()) device.setSwitchEnabled(device.getSwitchScheduler().getCurrentSchedule(), true);
                        r.getPersistance().saveDevice(device);
                        sendAjaxAnswer("OK");                        
                    }
                } else {
            ...
            } else if (action.equals("ALARMONMOTION")) {
                if (user.getGroup().equals(Group.OPERATOR_GROUP) || user.getGroup().equals(Group.ADMIN_GROUP)) {
                    if (requestParameters.get("all").equals("true")) {
                        for (AbstractDevice dev : r.getDevices()) {
                            if (dev instanceof AccelerometerSensorDevice) {
                                for (int i=0; i<14;i++) {
                                    ((AccelerometerSensorDevice)dev).setAlarmEnabled(true, true);
                                    r.getPersistance().saveDevice(((AccelerometerSensorDevice)dev));
                                }                                                                                        
                            }
                        }
                        ...
                        device.setAlarmEnabled(true, true);
                        r.getPersistance().saveDevice(device);
                        sendAjaxAnswer("OK");                        
                    }
                } else {
            ...
            } else if (action.equals("ALARMONORIENTATION")) {
                if (user.getGroup().equals(Group.OPERATOR_GROUP) || user.getGroup().equals(Group.ADMIN_GROUP)) {
                    if (requestParameters.get("all").equals("true")) {
                        for (AbstractDevice dev : r.getDevices()) {
                            if (dev instanceof AccelerometerSensorDevice) {
                                for (int i=0; i<14;i++) {
                                    ((AccelerometerSensorDevice)dev).setAlarmEnabled(true, false);
                                    r.getPersistance().saveDevice(((AccelerometerSensorDevice)dev));
                                }                                                                                        
                            }
                        }
                        ...
                        device.setAlarmEnabled(true, false);
                        r.getPersistance().saveDevice(device);
                        sendAjaxAnswer("OK");                        
                    }
                } else {
                ...
                if (user.getGroup().equals(Group.OPERATOR_GROUP) || user.getGroup().equals(Group.ADMIN_GROUP)) {
                    if (requestParameters.get("all").equals("true")) {
                        for (AbstractDevice dev : r.getDevices()) {
                            if (dev instanceof AccelerometerSensorDevice) {
                                for (int i=0; i<14;i++) {
                                    ((AccelerometerSensorDevice)dev).setAlarmEnabled(false, false);
                                    r.getPersistance().saveDevice(((AccelerometerSensorDevice)dev));
                                }
                        ...
                        device.setAlarmEnabled(false, false);
                        r.getPersistance().saveDevice(device);
                        sendAjaxAnswer("OK");                        
                    }
                } else {
            ...
            } else if (action.equals("SWITCHONMOTION")) {
                if (user.getGroup().equals(Group.OPERATOR_GROUP) || user.getGroup().equals(Group.ADMIN_GROUP)) {
                    if (requestParameters.get("all").equals("true")) {
                        for (AbstractDevice dev : r.getDevices()) {
                            if (dev instanceof AccelerometerSensorDevice) {
                                for (int i=0; i<14;i++) {
                                    ((AccelerometerSensorDevice)dev).setSwitchEnabled(true, true);
                                    r.getPersistance().saveDevice(((AccelerometerSensorDevice)dev));
                                }                                                        
                            }
                        }
                        ...
                        device.setSwitchEnabled(true, true);
                        r.getPersistance().saveDevice(device);
                        sendAjaxAnswer("OK");                        
                    }
                } else {
            ...
            } else if (action.equals("SWITCHONORIENTATION")) {
                if (user.getGroup().equals(Group.OPERATOR_GROUP) || user.getGroup().equals(Group.ADMIN_GROUP)) {
                    if (requestParameters.get("all").equals("true")) {
                        for (AbstractDevice dev : r.getDevices()) {
                            if (dev instanceof AccelerometerSensorDevice) {
                                for (int i=0; i<14;i++) {
                                    ((AccelerometerSensorDevice)dev).setSwitchEnabled(true, false);
                                    r.getPersistance().saveDevice(((AccelerometerSensorDevice)dev));
                                }                                                        
                            }
                        }
                        ...
                        device.setSwitchEnabled(true, false);
                        r.getPersistance().saveDevice(device);
                        sendAjaxAnswer("OK");                        
                    }
                } else {
                ...
                if (user.getGroup().equals(Group.OPERATOR_GROUP) || user.getGroup().equals(Group.ADMIN_GROUP)) {
                    if (requestParameters.get("all").equals("true")) {
                        for (AbstractDevice dev : r.getDevices()) {
                            if (dev instanceof AccelerometerSensorDevice) {
                                for (int i=0; i<14;i++) {
                                    ((AccelerometerSensorDevice)dev).setSwitchEnabled(false, false);
                                    r.getPersistance().saveDevice(((AccelerometerSensorDevice)dev));
                                }                                                        
                            }
                        }
                        ...
                        device.setSwitchEnabled(false, false);
                        r.getPersistance().saveDevice(device);
                        sendAjaxAnswer("OK");                        
                    }
                } else {
