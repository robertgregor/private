/**
  ...
  *         state - 0 = no movement detected, 1 = movement detected
...
public class PirSensorDevice extends AbstractDevice implements Serializable {
   /**
     * Current state of the PirSensor. 0 - no movement, 1 - movement detected, 2 - unknown
     */
    private int pirState; 

   /**
     * Internal state of the pir sensor for the history data purpose. 0 - no movement, 1 - movement detected, 2 - unknown
     */
    private int internalPirState;
    ...
    protected PirSensorDevice(RemoteHomeManager m, int deviceId, String deviceName) {
    ...
    /**
      ...
      * 3|frequency|battery|enabled|state
      */
    @Override
    protected void manageAsynchronousCommand(String[] items) {
        if (items[0].equals("3")) {
            ...
            setPirState(Integer.parseInt(items[4]));
    ...
    protected void saveHistoryData() throws RemoteHomeManagerException {
          int value = (!enabled)?2:internalPirState;
          HistoryData history = new HistoryData();
          history.setDeviceId(getDeviceId());
          history.setDataName("PIRSENSOR");
          history.setDataValue(value+"|"+(isEnabled()?0:1)+"|"+(isAlarmEnabled()?0:1)+"|"+(isIntruded()?0:1));
    ...
    /**
     * @return the pirState of the pirSensor. 0 - no movement, 1 - movement detected, 2 - unknown
     */
    ...
    /**
     * @param pirState the pirState to set
     */
    private void setPirState(int pirState) {
    ...
    */
    ...
    public class PirSensorChartItem {
    ...
    public ArrayList generateChartItems(HistoryData[] historyData, String type) {
        ArrayList<PirSensorDevice.PirSensorChartItem> retArray = new ArrayList<PirSensorDevice.PirSensorChartItem>();
            DateFormat df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'.000Z'");
            for (HistoryData d : historyData) {
                PirSensorDevice.PirSensorChartItem item = new PirSensorDevice.PirSensorChartItem();            
